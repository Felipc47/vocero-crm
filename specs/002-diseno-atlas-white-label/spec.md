...
- Seomos es genérico: cada instancia lo opera un negocio distinto → la marca
  visible (nombre + acento) debe ser suya, no "Seomos".
...
4. Filtros: "Todas" y "No leídas" con conteos (Seomos no tiene asignación de
...
**Aceptación**:
1. Nueva pestaña Configuración → Marca: input de nombre (default "Seomos") y
...
4. Sin configurar → defaults (nombre "Seomos", acento azul acero `#3f5972`).
...
Seomos), logo por imagen (v1: inicial sobre acento), densidad configurable.
